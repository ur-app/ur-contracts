...
contract Fiat24CryptoDeposit2 is OAppUpgradeable,OAppOptionsType3Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable,IFiat24CryptoDeposit {
    ...
    //UNISWAP ADDRESSES MONAD MAINNET
    address public constant UNISWAP_FACTORY = 0x204FAca1764B154221e35c0d20aBb3c525710498;
    address public constant UNISWAP_ROUTER = 0xfE31F71C1b106EAc32F1A19239c9a9A72ddfb900;
    address public constant UNISWAP_PERIPHERY_PAYMENTS = 0xfE31F71C1b106EAc32F1A19239c9a9A72ddfb900;
    address public constant UNISWAP_QUOTER = 0x661E93cca42AfacB172121EF892830cA3b70F08d;
    ...
    /// @param _inputToken The token to deposit (address(0) or 0xEeee...eEeE for ETH)
    /// @param _outputToken The target fiat24 token (EUR24, USD24, etc.)
    /// @param _amount The amount of input token to deposit (for ETH, this is the ETH amount to swap)
    ...
    /// @param _inputToken The token to deposit (address(0) or 0xEeee...eEeE for ETH)
    /// @param _outputToken The target fiat24 token (EUR24, USD24, etc.)
    /// @param _amount The amount of input token to deposit (for ETH, msg.value should equal _amount)
    /// @param _aggregator The whitelisted aggregator contract address
    /// @param _swapCalldata The calldata for the aggregator swap
    /// @param _minUsdcAmount The minimum acceptable USDC amount after swap
    ...
    function _swapToUsdc(
        ...
        // Execute swap
        if (_isETH) {
            usdcAmount = IV3SwapRouter(UNISWAP_ROUTER).exactInputSingle{value: _amount}(params);
            if (usdcAmount == 0) revert Fiat24CryptoDeposit__SwapOutputAmountZero();
            // Refund excess ETH
            ...
            usdcAmount = IV3SwapRouter(UNISWAP_ROUTER).exactInputSingle(params);
            if (usdcAmount == 0) revert Fiat24CryptoDeposit__SwapOutputAmountZero();
        }
