...
contract F24 is ERC20, ERC20Permit, ERC20Votes, ERC20Pausable, ERC20Burnable, AccessControl {
    ...
    constructor(address admin, address fiat24accountProxyAddress, uint256 maxSupply_, uint256 airdropTotal_, uint256 airdropEndTime_, uint256 airdropClaim_)
        ERC20("Fiat24", "F24")
        ERC20Permit("Fiat24")
    {
        require(airdropTotal_ <= maxSupply_, "F24: Airdrop higher than max supply - free supply");
        maxSupply = maxSupply_;
        _mint(admin, maxSupply_ - airdropTotal_);
    ...
    function claimToken(uint256 tokenId) external {
        ...
        require(fiat24account.ownerOf(tokenId) == msg.sender || fiat24account.historicOwnership(msg.sender) == tokenId, "F24: Not owner of token");
        require(
            fiat24account.status(tokenId) == Fiat24Account.Status.Live || fiat24account.status(tokenId) == Fiat24Account.Status.Tourist,
            "F24: Not Live or Tourist"
        );
    ...
    function eligibleClaimAmount(uint256 tokenId) public view returns (uint256) {
        ...
        if (fiat24account.exists(tokenId)) {
            if (fiat24account.status(tokenId) == Fiat24Account.Status.Live || fiat24account.status(tokenId) == Fiat24Account.Status.Tourist) {
