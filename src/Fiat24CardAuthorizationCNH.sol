...
contract Fiat24CardAuthorizationCNH is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    using DigitsOfUint for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    string public constant AUTHORIZER_CURRENCY = "CNH";
    ...
    function initialize(address admin, address fiat24AccountAddress_, address cnh24Address_, address eur24Address_, address f24Address_) public initializer {
    ...
    function authorize(string memory authorizationId_, string memory cardId_, uint256 tokenId_, address sender_, uint256 amount_) public {
        ...
        IERC20Upgradeable(cnh24Address).safeTransferFrom(sender_, IFiat24Account(fiat24AccountAddress).ownerOf(CARD_BOOKED), amount_);
    ...
    function reverse(
        ...
        if (!(hasRole(AUTHORIZER_ROLE, _msgSender()))) revert Fiat24CardAuthorization__NotAuthorizer(_msgSender());
        if (paused()) revert Fiat24CardAuthorization__Suspended();
        IERC20Upgradeable(cnh24Address).safeTransferFrom(IFiat24Account(fiat24AccountAddress).ownerOf(CARD_BOOKED), sender_, amount_);
